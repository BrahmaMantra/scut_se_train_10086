...
object CalRegionUser {
  def main(args: Array[String]): Unit = {
    ...
    import org.apache.flink.api.scala._ val prop = new Properties()
    ...
    val userInfoStream = env.readFile(new TextInputFormat(new Path(userInfoPath)), userInfoPath, FileProcessingMode
      .PROCESS_CONTINUOUSLY, 1000).map(x => UserInfo.fromStr(x))


    //获取基站、区域关系数据
    val regionCellPath = "hdfs://bigdata01:9000/data/regionCell"
    val regionCellStream = env.readFile(new TextInputFormat(new Path(regionCellPath)), regionCellPath,
      FileProcessingMode.PROCESS_CONTINUOUSLY, 1000).map(x => RegionCell.fromStr(x))


    val broadcastUserInfoDes = new MapStateDescriptor[String, UserInfo]("broadcastUserInfoDes", classOf[String],
      classOf[UserInfo])

    val userXdrStream = xdr.connect(userInfoStream.broadcast(broadcastUserInfoDes)).process(new
        BroadcastProcessFunction[XDR, UserInfo, UserXdr] {
      override def processElement(in1: XDR, readOnlyContext: BroadcastProcessFunction[XDR, UserInfo,
        UserXdr]#ReadOnlyContext, collector: Collector[UserXdr]): Unit = {
        val imsi = in1.imsi
      ...
      override def processBroadcastElement(in2: UserInfo, context: BroadcastProcessFunction[XDR, UserInfo,
        UserXdr]#Context, collector: Collector[UserXdr]): Unit = {
    ...
    val broadcastRegionCellDes = new MapStateDescriptor[String, String]("broadcastRegionCellDes", classOf[String],
      //laccell
    ...
    val userRegionFlowStream = userXdrStream.keyBy(_.imsi).connect(regionCellStream.broadcast(broadcastRegionCellDes)
      ...
      override def open(parameters: Configuration): Unit = {
        ...
        val lastRegionDes = new ValueStateDescriptor[String]("lastRegion", createTypeInformation[String])
        lastRegion = getRuntimeContext.getState(lastRegionDes)
      }

      override def processElement(in1: UserXdr, readOnlyContext: KeyedBroadcastProcessFunction[String, UserXdr,
        RegionCell, UserRegionFlow]#ReadOnlyContext, collector: Collector[UserRegionFlow]): Unit = {
        val laccell = in1.laccell
      ...
      override def processBroadcastElement(in2: RegionCell, context: KeyedBroadcastProcessFunction[String, UserXdr,
        RegionCell, UserRegionFlow]#Context, collector: Collector[UserRegionFlow]): Unit = {
    ...
    val regionCalStream = userRegionFlowStream.keyBy(_.regionId).process(new KeyedProcessFunction[String,
      UserRegionFlow, RegionCal]() {
      ...
      override def processElement(value: UserRegionFlow, context: KeyedProcessFunction[String, UserRegionFlow,
        RegionCal]#Context, collector: Collector[RegionCal]): Unit = {
      ...
      override def onTimer(timestamp: Long, ctx: KeyedProcessFunction[String, UserRegionFlow,
        RegionCal]#OnTimerContext, out: Collector[RegionCal]): Unit = {
    ...
    val hbaseSink = new HBaseSinkFunction[RegionCal]("user_persona", conf, new HBaseMutationConverter[RegionCal] {
      //初始化方法，只执行一次
