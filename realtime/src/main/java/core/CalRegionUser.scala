...
object CalRegionUser {
  def main(args: Array[String]): Unit = {
    ...
    val kafkaConsumer =
      new FlinkKafkaConsumer[String]("xdr", new SimpleStringSchema(), prop)
    ...
    val userXdrStream = xdr
      .connect(userInfoStream.broadcast(broadcastUserInfoDes))
      .process(new BroadcastProcessFunction[XDR, UserInfo, UserXdr] {
        override def processElement(
            in1: XDR,
            readOnlyContext: BroadcastProcessFunction[
              XDR,
              UserInfo,
              UserXdr
            ]#ReadOnlyContext,
          ...
          val broadcastState =
            readOnlyContext.getBroadcastState(broadcastUserInfoDes)
      ...
      .process(
        new KeyedBroadcastProcessFunction[
          ...
          override def processElement(
            ...
            val broadcastState =
              readOnlyContext.getBroadcastState(broadcastRegionCellDes)
            ...
            if (!regionId.equals(lastRegion.value()) && regionId.length > 0) {
              ...
              if (lastRegion.value() != null) {
                collector
                  .collect(UserRegionFlow(0, lastRegion.value(), gender, age))
              }
          ...
          override def processBroadcastElement(
              in2: RegionCell,
              context: KeyedBroadcastProcessFunction[
                String,
                UserXdr,
                RegionCell,
                UserRegionFlow
            ...
            val broadcastState =
              context.getBroadcastState(broadcastRegionCellDes)
    ...
    val regionCalStream = userRegionFlowStream
      ...
      .process(new KeyedProcessFunction[String, UserRegionFlow, RegionCal]() {
        ...
        override def processElement(
          ...
          val regionCal = Option(cal.value())
            .getOrElse(RegionCal(value.regionId, 0, 0, 0, 0, 0))
      ...
      new HBaseMutationConverter[RegionCal] {
        ...
        override def convertToMutation(record: RegionCal): Mutation = {
          ...
          put.addColumn(
            "persona".getBytes(),
            "man".getBytes(),
            value.manNum.toString.getBytes()
          )
          put.addColumn(
            "persona".getBytes(),
            "women".getBytes(),
            value.womanNum.toString.getBytes()
          )
          put.addColumn(
            "persona".getBytes(),
            "age_10_20".getBytes(),
            value.age_10_20.toString.getBytes()
          )
          put.addColumn(
            "persona".getBytes(),
            "age_20_40".getBytes(),
            value.age_20_40.toString.getBytes()
          )
          put.addColumn(
            "persona".getBytes(),
            "age_40".getBytes(),
            value.age_40.toString.getBytes()
          )
          put
        }
      },
