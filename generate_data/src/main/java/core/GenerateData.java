...
public class GenerateData {
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        Map<Integer, CellInfo> cellindex_cell_Map = cellLocLines.stream().collect(
                Collectors.toMap(x -> x.index, x -> x));
        Map<String, Integer> cell_cellindex_Map = cellLocLines.stream().collect(
                Collectors.toMap(x -> x.cell, x -> x.index));
        ...
        while (true) {
            ...
            int lastIndex = cell_cellindex_Map.getOrDefault(userStatus.get(imsi),
                                                            (Integer) r.nextInt(cellLocLines.size())
            );

            int step = r.nextInt(7) - 3;

            if (step > 0) {
                int[] chose = {lastIndex + step < cellLocLines.size() ? lastIndex + step : lastIndex - step,
                        lastIndex - step > 0 ? lastIndex - step : lastIndex + step};
                Integer nextIndex = chose[r.nextInt(2)];
    ...
    private static List<String> getDataFromFile(String filePath) throws IOException {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(
                Objects.requireNonNull(GenerateData.class.getClassLoader().getResourceAsStream(filePath))))
        ) {
            String line;
