...
object CalRegionUserStay {
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession = SparkSession.builder().appName("CalRegionUserStay").config(conf).getOrCreate()
    ...
    val xdrDf= sparkSession.read.format("csv").option("sep", "|").schema(xdrStruct)
        .load(xdrPath)
    ...
    val regionCellStruct = new StructType()
      ...
      .add("laccell", StringType)
    val regionCellDf = sparkSession.read.format("csv").option("sep", "|").schema(regionCellStruct)
      .load(regionCellPath)
    ...
    sparkSession.sql(sql).rdd.foreachPartition(it => {
      ...
      it.foreach(row => {
        ...
        val stmt = conn.prepareStatement(s"insert into REGION_ID_IMSI_BITMAP " +
