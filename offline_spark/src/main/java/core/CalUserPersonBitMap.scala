...
object CalUserPersonBitMap {
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession=SparkSession.builder().appName("CalUserPersonBitMap").config(conf).getOrCreate()
    ...
    val userInfoDf=sparkSession.read.format("csv").option("sep","|").schema(userInfoStruct)
    ...
    sparkSession.sql(sql).rdd.foreachPartition(it=>{
      ...
      it.foreach(row=>{
        ...
        val stmt=conn.prepareStatement(s"insert into TA_PORTRAIT_IMSI_BITMAP " +
